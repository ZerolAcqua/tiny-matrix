...
bool operator==(const Matrix &lMat, const Matrix &rMat)
{
	...
	if (count != rMat.miRow * rMat.miCol || lMat.miRow != rMat.miRow)
		return false;
...
Matrix Matrix::rMultiple(const Matrix &tmp) const
{
	...
	if (this->miRow <= 0 || this->miCol <= 0 || tmp.miRow <= 0 || tmp.miCol <= 0)
	{
		return Matrix();
	}
...
Matrix Matrix::eigen() const
{
	...
	for (int i = 0; i < ITER_TIMES; i++)
	{
		...
		if (count == col - 1)
		{
			...
			// 排序不想写了，直接用库(
			std::sort(result.mpBuf, result.mpBuf + result.miRow, [](int i, int j) -> bool
					  { return i > j; });
			return result;
		}
	}
